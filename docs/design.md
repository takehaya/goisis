...
| No graceful restart (RFC 5306) | A peer that crash-restarts and re-originates at sequence 1 is out-shouted by our stored higher-seq copy until it ages out. Under the RFC 7987 floor that is MaxAge (1200s) counted from when *this* node installed its copy, not whatever the originator had left — the normal case now, not a ceiling, and a lifetime advertised above MaxAge is kept as advertised, which stretches it further. It stays bounded rather than compounding: whichever node installed earliest reaches zero first and floods a purge the later ones accept at equal sequence. Clean shutdowns purge, so this affects only ungraceful restarts. |
| No BFD | Failure detection is hello-based (hold time). |
| No runtime circuit add | Prefixes, locators, Flex-Algos, the overload bit and adjacency resets change at runtime, and a circuit's addresses and carrier are followed live (`SetCircuitAddresses` / `SetCircuitLinkState`). A circuit can also be removed: `DeleteCircuit` takes its adjacencies down, purges the pseudonode LSPs it owned, withdraws the subnets it contributed and closes its transport. Adding one still needs a restart — the transport and the reader goroutine it wants are a startup's to create — as do the System ID, the area and an authentication key; accept lists (`*-accept-passwords`) make a key change a rolling restart rather than a flag day. `SIGHUP` re-reads the configuration file and applies the runtime set, naming every difference it leaves behind, and every circuit difference is still among them: the reload does not issue `DeleteCircuit` itself. |
...
| No RFC 7987 §3.2 corruption reporting | The floor itself is implemented (see **LSDB** above): a received lifetime below MaxAge is stored as MaxAge, one above it is kept as advertised because the originator may run a larger MaxAge (§3.1), and the floor is fixed at MaxAge rather than configurable. What is missing is the `CorruptRemainingLifetime` event: §3.2's false-positive filter needs how long the receiving adjacency has been Up, which the update process does not carry down to the LSP it is installing. Each repair is counted per circuit instead (`goisis_lsp_lifetime_floored_total`), which is not that event and does not stand in for it: ordinary aging is floored identically, so the counter's baseline is the circuit's LSP arrival rate and no value of it can be alerted on. It is usable only as one circuit's rate against the other circuits of the node; see [configuration.md](configuration.md). |
