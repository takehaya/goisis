...
- **管理プレーン。** Connect API は認証なしの平文 h2c で、デフォルトは
  ...
  `-api-allow-remote` オプトインが必要です。ローカルアクセスには
  `-api-listen unix:///run/goisis/goisisd.sock` を推奨します。ソケットは
  モード `0660` で作られ、ファイルシステムのパーミッションがアクセス制御に
  なります。ネットワーク越しに公開する場合は外部での保護（TLS プロキシ、
  ネットワークポリシー）が必要です。
