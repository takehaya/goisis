...
| グレースフルリスタート（RFC 5306）なし | クラッシュ再起動してシーケンス 1 から再生成するピアは、こちらの保存済み高シーケンスコピーに負け、エージアウトするまで無視されます。RFC 7987 の下限により、その時間は生成元の残り寿命ではなく「このノードが格納した時点から MaxAge（1200 秒）」です。上限ではなく通常のケースになりました。MaxAge を超える lifetime はそのまま保持するので、さらに延びることもあります。ただし際限なく延びるわけではありません。最初に格納したノードが最初にゼロへ達してパージをフラッディングし、後から格納したノードは同一シーケンスのパージとしてそれを受け入れます。クリーンな停止はパージするので、影響するのは非正常な再起動のみです。 |
