...
| サーキットの実行時追加・削除と設定リロードなし | プレフィックス、locator、Flex-Algo、オーバーロードビット、隣接のリセットは実行時に変更でき、サーキットのアドレスとキャリアは実時間で追従します（`SetCircuitAddresses` / `SetCircuitLinkState`）。サーキットの追加・削除と認証鍵の変更は依然として再起動が必要です — accept リスト（`*-accept-passwords`）により、鍵の変更は一斉切り替えではなくローリング再起動で済みます。 |
