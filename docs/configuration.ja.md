...
変えません。最初の呼び出しを出す前にファイル全体を検証するからです。`-f` なしで
起動したデーモンへの `SIGHUP` は致命的にせず無視します。オーバーロードビットは
そもそもファイルのキーではなく、実行時に `goisis overload on` で設定します。

検証で先回りできないのは、動いているサーバ自身が拒否する呼び出しです。リロードに
ロールバックはない(全呼び出しの逆操作が要る)ので、拒否された時点で止まり、その
ことをログに出します。ファイルを適用済みとして記録することはしません:
...
動作中の設定として扱うことはありません。拒否の報告が出なくなるまで信号を繰り返して
