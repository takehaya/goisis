...
| 制約 | 備考 |
|------|------|
| 単一エリア | L1/L2 の隣接とレベル別 SPF は動き、L1 のみのノードは最も近い attached な L1L2 IS へのデフォルト経路を入れます（ATT ビット、RFC 1195 §3.2）。経路リーク自体は双方向とも未実装で、L1→L2 の伝播も L2→L1 のリークもありません。up/down ビット（RFC 5305 §4.1 / RFC 5308 §2）はパース・保持されますが、生成時に立てることはありません。 |
