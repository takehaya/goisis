...
| サーキットの実行時追加なし | プレフィックス、locator、Flex-Algo、オーバーロードビット、隣接のリセットは実行時に変更でき、サーキットのアドレスとキャリアは実時間で追従します（`SetCircuitAddresses` / `SetCircuitLinkState`）。削除もできます: `DeleteCircuit` はそのサーキットの隣接を落とし、保持していた pseudonode LSP を purge し、寄与していたサブネットを取り下げ、トランスポートを閉じます。追加は依然として再起動が必要です — トランスポートとリーダ goroutine の用意は起動時の仕事だからです。System ID・エリア・認証鍵の変更も同じく再起動が必要で、accept リスト（`*-accept-passwords`）により鍵の変更は一斉切り替えではなくローリング再起動で済みます。`SIGHUP` は設定ファイルを読み直して実行時に反映できる範囲を反映し、反映しなかった差分はすべて名指しします。サーキットの差分は削除も含めてそこに残ります: リロード経路はまだ `DeleteCircuit` を呼びません。 |
