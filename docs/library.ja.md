...
`Constraints` です。このときそのアルゴリズムへの参加広報も取り下げます
(RFC 9350 §5.3)。計算していないノードにエリアの他のノードがそのアルゴリズムの
トラフィックを流し込まないようにするためです。そのレベルで誰も FAD を広報しておらず
`Definition` が nil の場合も、同じ一文のもう一方の条件に当たり、やはり参加広報は
行いません。
