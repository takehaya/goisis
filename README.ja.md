...
- **HMAC-MD5 認証**(RFC 5304)— hello と LSP/SNP の両方。FRR の `isis password md5` /
  `area-password md5` / `domain-password md5` と相互運用可能
...
| マイルストーン | 範囲 |
|-----------|-------|
| M0–M1 | スキャフォールド、CI/CD、PDU/TLV コーデック(fuzz・FRR golden) |
| M2–M3 | データリンク + 隣接 FSM、DIS 選出、LSP フラッディング、LSDB 同期 |
| M4 | SPF + RIB + prefix 広報 + netlink FIB |
| M5 | Connect RPC API + CLI + `WatchEvent` |
| M6 | SRv6 locator の広報・学習・End SID 設置 |
| M7 | Flex-Algo 定義・選出・アルゴリズム別 SPF |
| M8 | 起動時オーバーロード、クリーンシャットダウン purge、Prometheus メトリクス、HMAC-MD5 認証(hello + LSP/SNP) |

残り: HMAC-SHA(RFC 5310)、および API を `v1alpha1` から `v1` へ昇格する前提と
なる設定変更系 RPC。
