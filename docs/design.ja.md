...
- **LSDB。** レベルごとに 1 つ。エントリはデコード済み LSP と生バイトの
  ...
  §2）。パージはゼロ到達後 ZeroAgeLifetime の間保持されます。
...
| RFC 7987 §3.2 の破損報告なし | 下限そのものは実装済みです（上の **LSDB** 参照）: 受信 lifetime が MaxAge 未満なら MaxAge で保存し、MaxAge 超過の値は生成元がより大きな MaxAge で動作している可能性があるためそのまま保持し（§3.1）、下限値は設定可能にせず MaxAge 固定です。未実装なのは `CorruptRemainingLifetime` イベントです。§3.2 の誤検出フィルタには受信側の隣接が Up になってからの経過時間が必要ですが、更新プロセスはそれを LSP の格納処理まで持ち回っていません。代わりに補正の回数をサーキット単位で数えます（`goisis_lsp_lifetime_floored_total`）が、これはそのイベントの代わりにはなりません。通常のエージングも同じように補正されるため、カウンタの基準値はそのサーキットの LSP 受信レートそのものであり、どの値もアラートには使えません。使えるのは 1 サーキットのレートを同じノードの他のサーキットと比べたときだけです（[configuration.ja.md](configuration.ja.md) 参照）。 |
