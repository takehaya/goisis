...
- **ハウスキーピング（1 秒 tick）。** hello 送信、隣接の期限切れ、保留中
...
| サーキットの実行時追加・削除と設定リロードなし | プレフィックス、locator、Flex-Algo、オーバーロードビットは実行時に変更できます。サーキットや認証鍵の変更は再起動が必要です。 |
