...
hello の到着レートではなくそのレートになる。そして、1 つの隣接が自分の隣接が Up に
なってから次に Up になるまでの間に貯められる抑止は 5 分で頭打ちになり、そこを超えると
その隣接が何を要求してもフィルタは有効なままになる。頭打ちに達した瞬間に
`restart resync suppression budget spent` を 1 度だけログに出し、サーキット名、隣接、
隣接が Up でいる時間を添える — ここに達した隣接は、隣接を一度も Down させずに累計 5 分
データベースを渡されてきたということであり、再起動ループか、このカウンタを黙らせて
いる隣接かのどちらかである。頭打ちを超えると、その隣接では正規のヘルパ付き再起動も
数えられる。ログ行はそれを説明するためにある。隣接が一度 Down して Up に戻れば解除
される。頭打ちに達するまではアラートのベースラインは変わらない。
