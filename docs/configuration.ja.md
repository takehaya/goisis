...
ファイルが持てる YAML ドキュメントは 1 つです。`---` 区切りより後ろのドキュメントは
黙って捨てるのではなく拒否します。上記のチェックはデコードしたドキュメントしか見ない
ため、テンプレートやシークレット管理ツールが追記した正しい綴りの `area-password` が、
同じように何の報告もないままノードを認証なしにしていました。ファイル先頭の区切りは
2 つ目のドキュメントではないので、これまでどおり読み込めます。
...
ノードが参加するのは、列挙したアルゴリズムのうちそのレベルで定義を入手できている
ものだけで、SR-Algorithm sub-TLV 19 にもそれだけを載せます。RFC 9350 §5.3 が、有効な
定義がないアルゴリズムへの参加を取り下げることを求めているためです。したがって、
まだ誰も FAD を広報していない(あるいはこのレベルでは広報されていない)アルゴリズムは、
設定されていても広報しません。ここに列挙していないアルゴリズムに紐づく locator は
到達不能になるため、起動時に拒否されます。
...
| `circuits` | 反映。ファイルが足した回線は開き、外した回線は削除して pseudonode LSP を purge します。定義が変わった回線は削除して作り直すので、**隣接は落ちます**。落ちる前にリロードが回線名を挙げて警告します。開けなかったインタフェースが巻き添えにするのはその回線だけで、リロードは partial を報告し、次の `SIGHUP` が再試行します。ただしそのインタフェースができるまで、このファイルではデーモンは起動しません。アドレスとキャリアは実時間で追従するので、そもそもリロードは要りません。 |
...
距離で、アラートは上のログ行ではなくこちらに張ります。この値はリロードが
見送ったキーだけでなく、サーバが拒否した呼び出しも数えます。ノードを連れて
行けなかったファイルには、次回の起動も連れて行けないからです。
...
ファミリと束ねるアルゴリズム、prefix の形とメトリックの上限、そして回線自身の
フィールド、すなわち名前と DIS 優先度と hello 鍵がそれに当たります。最後の
グループの重みが一番大きく、定義の変わった回線は削除してから追加し直すため、
追加でしか通らない検査は回線を落としたあとに走ることになります。
...
ただし、これで助かるのは動作中のデーモンだけです。起動時にこの寛容さはなく、
ファイルが名指しした回線をすべて開き、開けないものが 1 つでもあればそこで
終了します。つまり NIC をファイルに書いてから NIC が現れるまでの間、ディスク上の
ファイルではデーモンは起動しません。無関係なクラッシュのあとに
`Restart=on-failure` が誰もいないところで試みる起動も同じで、systemd の 5 回で
諦め、ユニットは failed のまま残ります。この期間は
`goisis_config_reload_unapplied` がその回線を数え続けます。
...
リロードにロールバックはない(全呼び出しの逆操作が要る)ので、呼び出し一式は最後
まで出し切り、拒否されたものを名指ししてログに出します。ファイルを適用済みとして
記録することはしません。拒否が失うのはその呼び出しだけで、バッチの残りは通ります:
...
なく動作中のルーティング状態に対するアクセス制御です。書き込める人は、このノードの
IGP をこのホスト上のどのインタフェースにも取り付けられます。足された回線は設定済みの
area パスワードで hello を送り始め、残るログは `INFO` 1 行です。逆にファイルが名前を
書かなくなった回線は、その擬似ノード LSP をエリア全体へパージし、その上の隣接を
すべて落とします。同梱ユニットの `ExecReload=` により同じ経路が
`systemctl reload goisisd` からも届き、polkit は root を渡さずにこれを許可できます。
root 所有・モード `0640` 以下で配置してください。

認証も半分はこの範囲に入ります。hello の鍵は回線ごとの設定で、設定が変わった回線は
削除と追加の組になるため、`hello-password` を落としたファイルはその回線を hello HMAC
なしで作り直したうえで、リロードを適用済みとして報告します。0.5.0 まではこれに再起動が
必要で、再起動は少なくとも運用者に見える出来事でした。リロードが届かないままなのは
LSP と SNP の鍵で、`area-*` と `domain-*` のキーはいずれも拒否され、次回の再起動まで
持ち越されます。
...
RFC 5305 の到達不能しきい値 (`0xfe000000`) 以上のメトリックです。デフォルトルートは
拒否しません。広報自体は正当な手段で、抑止するなら `policy.advertise` を使います。
0.5.0 まではこれらを拒否するのが `prefix add` だけだったため、同じ内容を書いた
ファイルは当時は起動できました。
...
投げたものである。直近のリロードがファイルに残した差分の数、すなわち次の再起動が
出会うものの数を示す([設定のリロード](#設定のリロード))。再起動が拾ってしまう
キーと、再起動が失敗する呼び出し、たとえばインタフェースが存在しない回線の両方が
入る。2 つのファイルの比較まで到達したリロードでは、ゼロを含めて毎回更新するため、
解消した乖離は最後の値を保ったままにならずゼロとして読める。読み込みや検証で
拒否されたファイルは比較の手前で止まるのでこのゲージは動かない。そのときに動くのは
上のリロードカウンタである:
...
*どの*キーが待機中かを示すラベルは、意図的に持たせていない。このゲージが答えるのは
「動作中の設定はこのファイルか」であり、その答えは背後にあるキーが何個でも変わらない。
拒否したキーは 1 つずつ `WARN` に名指しで出るし、拒否された呼び出しはリロードが返す
エラーが名指しする。どれなのかを知りたい運用者が見るのはそちらであって、ラベルは
同じ名前を、個別にアラートしない時系列へ複製するだけになる。
...
止めているのがこの下限だが、同時に破損の唯一の症状も消している。

ただしこのカウンタは、その破損と通常のエージングを区別しない。絶対値には意味が
ない。生成元を出てから時間が経った LSP はすべて同じように引き上げられるため、
基準値はそのサーキットの LSP 受信レートそのものになる。通常の再フラッディングも、
リフレッシュも、新しい隣接が引き起こすデータベース全体の再同期 (これは一度に
まとめて) も、すべて数えられる。したがってアラートに使える閾値は存在しない。
「0 より大きい」も閾値にはならない。

使い方は比較である。隣接の変化がない時間帯で、1 サーキットのレートを同じノードの
他のサーキットと比べる。ただし、それぞれが運ぶフラッディング量の差は割り引く
(LAN は隣接が多いぶん、同じ LSP のコピーをポイントツーポイントより多く受け取る)。
そこで突出したまま戻らないサーキットがキャプチャを取る対象で、隣接が上がった
タイミングと一致するスパイクは再同期である。

```
rate(goisis_lsp_lifetime_floored_total[1h])   # 閾値ではなくサーキット間の比較に使う
```
...
メンテナンスで鳴る。そこで窓は隣接ではなく交換のほうに取ってある — このノードが始める
交換のたびに窓が開き直り、最後の交換から ZeroAgeLifetime で閉じる。2 つの
読み方が食い違うのは再起動を助けている間だけであり、それがあったことを示すのが
`goisis_restart_requests_total` である。

隣接が Up であった時間は単調で、隣接がそれを動かすことはできない。一方、交換はそうでは
ない — いつ要求するかを決めるのは隣接だからである。この境界を取り戻すものが 2 つある。
交換自体がサーキットとレベルごとに 5 秒に 1 回へ抑えられているので、窓が開き直すのは
hello の到着レートではなくそのレートになる。そしてもう 1 つが、隣接ごとに持つ 5 分の
抑止バジェットである。窓を開き直すたびに、その隣接の窓が前回開いてからの間隔を
ZeroAgeLifetime を上限として課金するので、隣接が要求した回数ではなく実際に抑止した
時間ぶんを支払うことになる — 静かな隣接での単発の 30 秒再起動なら 1 分半ほどで、その
大半は最初の 1 回ぶんの ZeroAgeLifetime である。バジェットが尽きている間は、その隣接が
何を要求してもフィルタは有効なままになる。

バジェットは経過時間の 10 分の 1 の速さで戻る。つまり 10 分静かなら 1 分ぶん回復し、
再起動ループから 1 時間経ったセグメントはベースライン 0 を取り戻し、要求をやめない
隣接が開かせられる窓は 10 回に 1 回 — アラートが評価する 1 時間の 10 分の 1 — になる。
隣接が一度 Down して Up に戻ればバジェットは満額に戻り、ブロードキャストサーキットでは
このノードをエコーしない hello が 1 つ来るだけでも同じことが起きる。ただしその代償は、
ログに残る 2 回の状態遷移と LSP の再生成である。

ブロードキャストサーキットでは、交換は要求した隣接のものではなくサーキットのもので
ある: §3.2.1c はレベル全体にフラッディングするので、1 つの隣接の再起動がそのレベルの
すべての隣接のバジェットを使う。頭打ちに達した瞬間に
`restart resync suppression budget spent by this circuit's exchanges` をバジェット
ごとに 1 度だけログへ出し、サーキット名、カウンタが尽きた隣接、その隣接が Up でいる
時間を添える。LAN ではその隣接が原因とは限らない。誰が要求したかを示す相関指標は
隣接単位ではなくサーキット単位のものである:

```
sum by (circuit) (goisis_restart_requests_total{outcome="held"})
```

頭打ちを超えると、その隣接では正規のヘルパ付き再起動も数えられる。ログ行はそれを
説明するためにある。頭打ちに達するまではアラートのベースラインは変わらない。
