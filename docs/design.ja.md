...
| RFC 7987 §3.2 の破損報告なし | 下限そのものは実装済みです（上の **LSDB** 参照）: 受信 lifetime が MaxAge 未満なら MaxAge で保存し、MaxAge 超過の値は生成元がより大きな MaxAge で動作している可能性があるためそのまま保持し（§3.1）、下限値は設定可能にせず MaxAge 固定です。未実装なのは `CorruptRemainingLifetime` イベントで、補正は黙って行われます。§3.2 の誤検出フィルタには受信側の隣接が Up になってからの経過時間が必要ですが、更新プロセスはそれを LSP の格納処理まで持ち回っていません。 |
