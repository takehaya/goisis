...
- **HMAC-MD5 hello 認証**(RFC 5304)— FRR の `isis password md5` と相互運用可能
...
| M8 | 起動時オーバーロード、クリーンシャットダウン purge、Prometheus メトリクス、HMAC-MD5 hello 認証 |

残り: LSP/SNP 認証(area/domain password)と HMAC-SHA(RFC 5310)、および API を
`v1alpha1` から `v1` へ昇格する前提となる設定変更系 RPC。
