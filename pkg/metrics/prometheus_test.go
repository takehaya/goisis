...
func TestForgetCircuitDropsEverySeriesThatNamesIt(t *testing.T) {
	...
	for _, c := range []string{"eth0", "eth1"} {
		if n := len(before[c]); n != 13 {
			t.Fatalf("%s has %d circuit-labelled series (%v), want the 13 collectors that carry the label: a collector was added without a line in this fixture or in ForgetCircuit", c, n, before[c])
		}
	}
